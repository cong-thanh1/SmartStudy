...
export class DynamoDbAiJobRepository implements IAiJobRepository {
  ...
  private async update(jobId: string, UpdateExpression: string, ExpressionAttributeNames: Record<string, string>, ExpressionAttributeValues: Record<string, unknown>): Promise<void> { await this.client.send(new UpdateCommand({ TableName: this.tableName, Key: { jobId }, UpdateExpression, ExpressionAttributeNames, ExpressionAttributeValues })); }
}
